...
module.exports = {
  name: "search",
  description: "Milyen számot keresel?",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**használat - **\`${GuildDB.prefix}search [zene]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let Pages = songs.map((songz) => {
        let MappedSongs = songz.map(
          (s) =>
            `\`${s.index + 1}.\` [${s.title}](${
              s.uri
            }) \nidő: \`${prettyMilliseconds(s.duration, {
              colonNotation: true,
            })}\``
        );

        let em = new MessageEmbed()
          .setAuthor("Rákerestem " + SearchString, client.botconfig.IconURL)
          .setColor("RANDOM")
      ...
      let msg = await message.channel.send(
        "**Egy számot írj be, vége lesz ekkor `30 másodperc`.**"
      );
      ...
      if (!Song) return client.sendTime(message.channel, "Mivan?");
