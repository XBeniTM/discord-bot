    ...
    switch (stateChange.type) {
        case "JOIN":
            if (stateChange.members.size === 1 && player.paused) {
                let emb = new MessageEmbed()
                    .setAuthor(`Zene folytatva ♪`, client.botconfig.IconURL)
                    .setColor("RANDOM")
                    .setDescription(`Folytatom a zenét, viszont az elöbb itt hagytál engem egyedül...`);
        ...
        case "LEAVE":
            if (stateChange.members.size === 0 && !player.paused && player.playing) {
                ...
                let emb = new MessageEmbed()
                    .setAuthor(`Leállítva ♪`, client.botconfig.IconURL)
                    .setColor("RANDOM")
                    .setDescription(`Kiléptél! (Testvérek voltunk és megmerted ezt csinálni.....)`);
                await client.channels.cache.get(player.textChannel).send(emb);
            }
            break;
    }
